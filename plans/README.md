...
| [09-run-workbench.md](09-run-workbench.md) | Run workbench | `/run` four-step wizard and `/runs/[id]` live arena: grid, SSE client with reconnect/rehydration. Cell detail moved to plan 15. | `app/run/page.tsx`, `app/runs/[id]/*`, `components/run/*`, `components/arena/*`, `lib/client/useRunStream.ts`, `lib/client/runStore.ts`, `lib/client/runDraft.ts` |
...
| [15-cell-page.md](15-cell-page.md) | Cell detail page | Dedicated `/runs/[id]/cell/[category]?candidate=` page replacing the cell drawer; URL helpers for OpenRouter ids. | `lib/cellRef.ts`, `app/runs/[id]/cell/[category]/*`, `components/arena/CellPage.tsx`, snapshot `tasks` fields |
| [16-chat-playground.md](16-chat-playground.md) | Chat playground | Free chat + classify/score judges, chat SSE, `/playground` + chat leaderboard, migration 004. | `lib/chat-engine.ts`, `lib/bundles/chat-rubrics.ts`, `app/api/chat/**`, `app/playground/**`, `components/playground/**` |
